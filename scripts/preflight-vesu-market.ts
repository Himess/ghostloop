import { ADDRESSES, CHAIN_ID, getProvider, getRpcUrl } from "./config.js";
...
export async function preflightVesuMarket() {
  ...
  const [blockNumber, pairConfigRaw, pairRaw, debtAssetRaw, debtPriceRaw] =
    await Promise.all([
  ...
  const priceIsValid = BigInt(debtPriceRaw[2] ?? "0") !== 0n;
  ...
  if (!priceIsValid || debtPrice === 0n) reasons.push("USDC oracle price is invalid");
